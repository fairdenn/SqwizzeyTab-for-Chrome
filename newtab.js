...
const ICONS = {
  ...
  settings: `<svg viewBox="0 0 512 512" aria-hidden="true"><path d="M262.29 192.31a64 64 0 1 0 57.4 57.4 64.13 64.13 0 0 0-57.4-57.4zM416.39 256a154.34 154.34 0 0 1-1.53 20.79l45.21 35.46a10.81 10.81 0 0 1 2.45 13.75l-42.77 74a10.81 10.81 0 0 1-13.14 4.59l-44.9-18.08a16.11 16.11 0 0 0-15.17 1.75A164.48 164.48 0 0 1 325 400.8a15.94 15.94 0 0 0-8.82 12.14l-6.73 47.89a11.08 11.08 0 0 1-10.68 9.17h-85.54a11.11 11.11 0 0 1-10.69-8.87l-6.72-47.82a16.07 16.07 0 0 0-9-12.22 155.3 155.3 0 0 1-21.46-12.57 16 16 0 0 0-15-1.75l-44.89 18.07a10.81 10.81 0 0 1-13.14-4.58l-42.77-74a10.8 10.8 0 0 1 2.45-13.75l38.21-30a16.05 16.05 0 0 0 6-14.08c-.36-4.17-.58-8.33-.58-12.5s.21-8.27.58-12.35a16 16 0 0 0-6.07-13.94l-38.2-30A10.81 10.81 0 0 1 49.48 186l42.77-74a10.81 10.81 0 0 1 13.14-4.59l44.9 18.08a16.11 16.11 0 0 0 15.17-1.75A164.48 164.48 0 0 1 187 111.2a15.94 15.94 0 0 0 8.82-12.14l6.73-47.89A11.08 11.08 0 0 1 213.23 42h85.54a11.11 11.11 0 0 1 10.69 8.87l6.72 47.82a16.07 16.07 0 0 0 9 12.22 155.3 155.3 0 0 1 21.46 12.57 16 16 0 0 0 15 1.75l44.89-18.07a10.81 10.81 0 0 1 13.14 4.58l42.77 74a10.8 10.8 0 0 1-2.45 13.75l-38.21 30a16.05 16.05 0 0 0-6.05 14.08c.33 4.14.55 8.3.55 12.46z"/></svg>`,
...
/* ====================================================
   SQWIZZEY SURGICAL BOARD UPDATE v2.9.75
   Обновляет ссылки ОДНОЙ доски без перерисовки всей сетки.
   ==================================================== */
function refreshBoardLinks(boardId) {
  let page;
  try { page = activePage(); } catch { return; }
  const board = page?.boards.find(b => b.id === boardId);
  if (!board) return;

  const boardNode = document.querySelector(`.board[data-board-id="${boardId}"]`);
  if (!boardNode) { renderBoards(); return; } // доски нет в DOM — полный рендер

  const linkList = boardNode.querySelector(".link-list");
  if (!linkList) { renderBoards(); return; }

  // Перерисовываем только список ссылок этой доски
  linkList.innerHTML = renderBoardLinks(board);

  // Перевешиваем обработчики edit/delete для новых ссылок
  linkList.querySelectorAll("[data-link-action]").forEach(button => {
    const linkEl = button.closest(".bookmark-link");
    if (!linkEl) return;
  ...
  // Перевешиваем "Показать ещё / Свернуть"
  linkList.querySelectorAll("[data-board-toggle]").forEach(button => {
    button.addEventListener("click", () => {
      const action = button.dataset.boardToggle || "expand";
      if (action === "collapse") expandedBoardsRuntime.delete(board.id);
      else expandedBoardsRuntime.add(board.id);
      renderBoards();
    });
  });
}
...
/* ====================================================
   SQWIZZEY BOOKMARK SYNC v2.9.74
   Папка закладок Chrome = доска.
  ...
  folders.forEach(folder => {
    ...
    folder.links.forEach(link => {
      if (!existingUrls.has(link.url)) {
        board.links.push({ id: uid("link"), title: link.title, url: link.url });
...
// Возвращает все папки закладок с ТОЛЬКО ПРЯМЫМИ ссылками (без рекурсии),
// чтобы каждая папка = доска без дублей между родителем и потомком
async function getDirectFolderLinks() {
  ...
  function walk(node) {
    ...
    // Пропускаем корни без названия только если у них нет прямых ссылок
    if (directLinks.length && node.id !== "0") {
      folders.push({
        id: node.id,
        title: node.title || "Панель закладок",
        links: directLinks.map(c => ({ title: c.title || c.url, url: c.url }))
      });
    }
    node.children.filter(c => !c.url).forEach(walk);
  }

  (roots?.[0]?.children || []).forEach(walk);
  return folders;
}
...
function bindBookmarkListeners() {
  ...
  chrome.bookmarks.onCreated.addListener(async (id, bookmark) => {
    ...
    await persist();
    refreshBoardLinks(board.id);
  });

  // Закладка удалена в Chrome
...
// Запуск синхронизации после инициализации
queueMicrotask(() => {
  setTimeout(() => {
    try {
      bindBookmarkListeners();
      syncBookmarksToBoards({ createMissing: false });
    } catch (e) {
      console.warn("bookmark sync init failed", e);
    }
  }, 300);
});
